...
class CKDPatchExtension(APPatchExtension):
...
class CKDProcedurePatch(APProcedurePatch, APTokenMixin):
    hash = [HASH_US]
    game = "yrtnuoC gnoK yeknoD"
    patch_file_ending = ".apckd"
    result_file_ending = ".sfc"
    name: bytearray
    procedure = [
        ("apply_tokens", ["token_patch.bin"]),
        ("apply_bsdiff4", ["ckd_basepatch.bsdiff4"]),
        ("swap_kong_letters", []),
    ]
...
def patch_rom(world: "CKDWorld", patch: CKDProcedurePatch):
    ...
    patch.name = bytearray(f'1CKD{__version__.replace(".", "")[0:3]}_{world.player}_{world.multiworld.seed:11}\0', 'utf8')[:21]
...
def adjust_palettes(world: "CKDWorld", patch: CKDProcedurePatch):
    palette_options = {
...
def get_base_rom_path(file_name: str = "") -> str:
    ...
    if not file_name:
        file_name = options["ckd_options"]["rom_file"]
