...
from .Locations import CKDLocation
from .Items import CKDItem
...
def create_regions(world: "CKDWorld", active_locations):
    ...
    if world.options.banana_checks:
        ...
        add_location_to_region(multiworld, player, active_locations, RegionName.slipslide_ride_level, LocationName.slipslide_ride_bunch_10)
        add_location_to_region(multiworld, player, active_locations, RegionName.slipslide_ride_level, LocationName.slipslide_ride_bunch_11)
        ...
        add_location_to_region(multiworld, player, active_locations, RegionName.poison_pond_level, LocationName.poison_pond_bunch_7)
...
def connect_regions(world: "CKDWorld"):
...
def add_event_to_region(multiworld: MultiWorld, player: int, region_name: str, event_name: str, event_item=None):
    ...
    event = CKDLocation(player, event_name, None, region)
    if event_item:
        event.place_locked_item(CKDItem(event_item, ItemClassification.progression, None, player))
    else:
        event.place_locked_item(CKDItem(event_name, ItemClassification.progression, None, player))
...
def add_location_to_region(multiworld: MultiWorld, player: int, active_locations, region_name: str, location_name: str):
    ...
    if loc_id:
        location = CKDLocation(player, location_name, loc_id, region)
