...
class CKDSNIClient(SNIClient):
    ...
    async def validate_rom(self, ctx):
        ...
        if rom_name is None or setting_data is None or rom_name == bytes([0] * ROMHASH_SIZE) or rom_name[:4] != b"1CKD":
    ...
    async def game_watcher(self, ctx):
        ...
        if self.current_map != loaded_level:
            ...
            await ctx.send_msgs([{
                "cmd": "Set", 
                "key": f"ckd_current_map_{ctx.team}_{ctx.slot}", 
                "default": 0,
                "want_reply": False,
                "operations":
