...
from .Items import CKDItem, item_table, misc_table, item_groups, STARTING_ID, option_name_to_world_name, items_that_open_checks
...
class CKDSettings(settings.Group):
...
class CKDWeb(WebWorld):
    ...
    option_groups = ckd_option_groups


class CKDWorld(World):
    """
    eknom
    """
    game = "yrtnuoC gnoK yeknoD"
    web = CKDWeb()

    settings: ClassVar[CKDSettings]
    ...
    def create_item(self, name: str, force_classification=False) -> CKDItem:
        ...
        created_item = CKDItem(name, classification, data.code, self.player)
    ...
    def generate_early(self):
        ...
        if hasattr(self.multiworld, "re_gen_passthrough"):
            if "yrtnuoC gnoK yeknoD" in self.multiworld.re_gen_passthrough:
                self.using_ut = True
    ...
    def generate_output(self, output_directory: str):
        try:
            patch = CKDProcedurePatch(player=self.player, player_name=self.multiworld.player_name[self.player])
            patch.write_file("ckd_basepatch.bsdiff4", pkgutil.get_data(__name__, "data/ckd_basepatch.bsdiff4"))
